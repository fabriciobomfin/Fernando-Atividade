"""
Nome do arquivo: controle_galpao_view.py
...
# Simulação de leitura dos sensores (em um sistema real, seria integração com Arduino/IOT)
def ler_sensores():
...
def abrir_tela_controle_galpao():
    janela = Toplevel()
    janela.title("Controle de Galpão - Sensores")
    ...
    # Layout
    Label(janela, text="Monitoramento dos Sensores do Galpão", font=("Arial", 14, "bold")).pack(pady=15)
    ...
    Label(frame_atuadores, text="Controle dos Atuadores", font=("Arial", 14, "bold")).grid(row=0, column=0, columnspan=2, pady=10)
